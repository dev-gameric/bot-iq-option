...
def check_stop():
    ...
    if lucro_total >= float('-'+str(abs(stop_loss))):
        stop = False
        print('\n***************************************')
        print('\nSTOP WIN ',str(cifrao),str(lucro_total))
        sys.exit()
...
### função para buscar hora da corretora ###
def horario():
...
def estrategia_mhi():
    ...
    while True:
        ...
        if entrar:
            ...
            cores = velas[-7],velas[-6],velas[-5],velas[-4],velas[-3],velas[-2],velas[-1]
            ...
            if direcao == 'put' or direcao == 'call':
                print('Velas: ',velas[-7],velas[-6],velas[-5],velas[-4],velas[-3],velas[-2],velas[-1], ' - Entrada para ', direcao)
                compra(ativo,valor_entrada,direcao,1,tipo)
                print('\n')
            else:
                if direcao == 'abortar':
                    print('Velas: ',velas[-7],velas[-6],velas[-5],velas[-4],velas[-3],velas[-2],velas[-1])
                    print('Entrada abortada - contra tendencia.\n')
                else:
                    print('velas: ',velas[-7],velas[-6],velas[-5],velas[-4],velas[-3],velas[-2],velas[-1])
                    print('Entrada abortada - doji encontrado')
                time.sleep(2)
            print('\n******************************************************\n')
